...
- target leakage scan includes univariate associations plus a multivariate/interaction check by default when `X_ref` is available; proxies outside `X_ref` can still pass undetected

**Interpretation Note:** The label-permutation test refits models by default when refit
data are available (auto mode); otherwise it keeps predictions fixed, so the p-value
quantifies prediction-label association rather than a full refit null. A large gap
indicates a non-random association. To determine if that signal is *real* or *leaked*,
check the **Batch Association** (confounding), **Target Leakage Scan** (proxy features),
and **Duplicate Detection** (memorization) tables. Use `perm_refit = FALSE` to force
fixed-prediction shuffles or `perm_refit = TRUE` with `perm_refit_spec` to always refit.
...
include within-fold duplicates (data-quality checks). Use `ci_method = "bootstrap"`
with `boot_B` if you want a confidence interval for the permutation gap.
...
The duplicate table lists pairs of samples with near-identical profiles that cross train/test folds (by default). Notably, the top row flags the pair (1, 5) with a similarity score of 1.0.

This confirms that the audit successfully detected the artificial duplicate injected during setup (`X_ref[c(1, 5), ] <- X_ref[1, ]`). Use `duplicate_scope = "all"` to include within-fold duplicates.
