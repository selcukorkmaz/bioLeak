...
knitr::kable(utils::head(df, 6),
             caption = "Example data (first 6 rows).")
knitr::kable(as.data.frame(table(df$outcome)),
             col.names = c("outcome", "n"),
             caption = "Outcome class counts.")
```
...
knitr::kable(scaled_summary,
             caption = "Leaky global scaling: means ~0 and SDs ~1 computed on all samples.")
```
...
cat("\nGuardFit summary:\n")
summary(guard)
knitr::kable(utils::head(train_x_guarded, 5),
             caption = "Guarded training data (first 5 rows).")
knitr::kable(utils::head(test_x_guarded, 5),
             caption = "Guarded test data (first 5 rows).")
```
...
knitr::kable(level_state$data,
             caption = "Aligned factor data with consistent levels.")
cat("Levels map:\n")
level_state$levels
```
...
knitr::kable(data.frame(feature = names(all_median), median = all_median),
             caption = "Leaky medians computed on train + test.")
knitr::kable(train_leaky, caption = "Leaky-imputed training data.")
knitr::kable(test_leaky, caption = "Leaky-imputed test data.")
```
...
knitr::kable(imp$train, caption = "Guarded-imputed training data.")
knitr::kable(imp$test, caption = "Guarded-imputed test data.")
```
...
- custom learners via `custom_learners`
...
- custom learners must return one prediction per test row
- if you supply a matrix without metadata, you must remove leakage columns
  yourself (group, batch, study, time)

Use `learner_args` to pass model-specific arguments; for custom learners you can
supply separate `fit` and `predict` argument lists. Set `parallel = TRUE` to use
future.apply for fold-level parallelism when available. When `learner` is a
...
**Custom learner (base R logistic regression)**
...
`predict` function. These are the minimal interfaces `fit_resample()` expects.
...
knitr::kable(metrics_leaky,
             caption = "Leaky fit: mean and SD of metrics across folds.")
```
...
knitr::kable(metrics_safe,
             caption = "Leakage-safe fit: mean and SD of metrics across folds.")
knitr::kable(utils::head(fit_safe@metrics, 5),
             caption = "Per-fold metrics (first 5 rows).")
```
...
**Parsnip model specification (optional)**

```{r fit-parsnip}
if (requireNamespace("parsnip", quietly = TRUE)) {
  spec <- parsnip::logistic_reg() |>
    parsnip::set_engine("glm")
  fit_parsnip <- fit_resample(
    df,
    outcome = "outcome",
    splits = safe_splits,
    learner = spec,
  ...
  cat("Parsnip logistic regression summary:\n")
  summary(fit_parsnip)
} else {
  cat("Parsnip not installed; skipping parsnip example.\n")
}
```

The output mirrors the base learner summary but is produced through a
`parsnip` model specification. Use this pattern to swap in alternative models
...
if (!is.null(audit@permutation_gap) && nrow(audit@permutation_gap) > 0) {
  knitr::kable(audit@permutation_gap,
               caption = "Permutation significance results.")
}
if (!is.null(audit@batch_assoc) && nrow(audit@batch_assoc) > 0) {
  knitr::kable(audit@batch_assoc,
               caption = "Batch/study association with folds (Cramer's V).")
} else {
  cat("No batch or study associations detected.\n")
}
if (!is.null(audit@target_assoc) && nrow(audit@target_assoc) > 0) {
  knitr::kable(utils::head(audit@target_assoc, 5),
               caption = "Top features by target association score.")
} else {
  cat("No target leakage scan results available.\n")
}
if (!is.null(audit@duplicates) && nrow(audit@duplicates) > 0) {
  knitr::kable(utils::head(audit@duplicates, 5),
               caption = "Top duplicate/near-duplicate pairs by similarity.")
} else {
...
```{r audit-by-learner}
custom_multi <- list(
  glm = custom_learners$glm,
  glm_int = list(
    fit = function(x, y, task, weights, ...) {
      df_fit <- data.frame(y = y, x, check.names = FALSE)
      stats::glm(y ~ (.)^2, data = df_fit,
                 family = stats::binomial(), weights = weights)
    },
    predict = function(object, newdata, task, ...) {
      as.numeric(stats::predict(object, newdata = as.data.frame(newdata),
...
fit_multi <- fit_resample(
  ...
  outcome = "outcome",
  splits = safe_splits,
  learner = c("glm", "glm_int"),
  custom_learners = custom_multi,
  metrics = "auc"
)
...
if (!is.null(audit_time@permutation_gap) && nrow(audit_time@permutation_gap) > 0) {
  knitr::kable(audit_time@permutation_gap,
               caption = "Time-series permutation significance results.")
}
...
if (requireNamespace("glmnet", quietly = TRUE)) {
  ...
  knitr::kable(utils::head(sim, 5),
               caption = "Simulation results (first 5 rows).")
} else {
