...
These features are available in the current stable release:
...
The core estimand is the repeat-level mean performance gap:

$$\delta_{\text{metric}} = \overline{m}_{\text{naive}} - \overline{m}_{\text{guarded}}$$

where each $\overline{m}$ is a fold-size–weighted average within a repeat,
averaged across all repeats. For lower-is-better metrics (RMSE, MAE, log-loss,
…) the sign is flipped so that a **positive ΔLSI always indicates
leakage-induced inflation** regardless of direction.

ΔLSI is designed for paired designs where both pipelines run on identical fold
assignments, enabling a within-repeat paired comparison that eliminates
between-fold variability from the test statistic. An unpaired fallback is
available when fold structures differ.
...
`summary()` reports the tier, effective repeats, both point estimates, the
BCa CI (if available), and the sign-flip p-value (if available).

## Inference tiers

ΔLSI inference quality scales with the effective number of matched repeats
(`R_eff`). The tier system makes this dependency explicit and prevents
overconfident claims from sparse designs:

| Tier | `R_eff` | Point estimate | p-value | BCa CI | `inference_ok` |
|------|---------|---------------|---------|--------|---------------|
| `D_insufficient` | < 5  | ✓ | — | — | FALSE |
| `C_point_only`   | ≥ 5  | ✓ | ✓ | — | FALSE |
| `B_ci_only`      | ≥ 10 | ✓ | ✓ | ✓ | FALSE |
| `A_full_inference`| ≥ 20 | ✓ | ✓ | ✓ | TRUE  |

Plan your repeated CV design around the desired tier:

- **Tier C** (p-value only): 5-fold × 5 repeats → `R_eff = 5`
- **Tier B** (CI available): 5-fold × 10 repeats → `R_eff = 10`
- **Tier A** (full inference, `inference_ok = TRUE`): 5-fold × 20 repeats → `R_eff = 20`

For paired designs, `R_eff` equals the number of matched repeats. For unpaired
designs, `R_eff = 0` and only the raw delta_metric point estimate is available.
...
The sign-flip test evaluates H₀: δ = 0 (no leakage inflation). For each of
the `R_eff` repeat-level differences δᵣ, the test randomly flips their signs
and re-computes the mean. The p-value is the proportion of sign combinations
with |mean| ≥ |observed|, making it a two-sided test.

Two computation modes are used depending on `R_eff`:

- **Exact enumeration** (`R_eff ≤ 15`): all 2^R sign combinations are
  evaluated. The p-value is `count / 2^R` with **no** continuity correction —
  exact computation requires no correction because every combination (including
  the observed all-positive pattern) is enumerated.

- **Monte Carlo** (`R_eff > 15`): `M_flip` random sign vectors are sampled.
  The Phipson & Smyth (2010) `+1` correction — `p = (count + 1) / (M_flip + 1)`
  — is applied to ensure the p-value is a valid probability under the null,
  even in the extreme tail.

### Huber-robust location estimate (`delta_lsi`)

The `delta_lsi` slot uses the Huber M-estimator with scale fixed at the
initial median absolute deviation (MAD). This provides robustness against
outlier folds arising from unusually easy or difficult test partitions.
When MAD = 0 (all δᵣ identical), the estimator reduces to the median, which
equals the arithmetic mean in that case. The raw mean `delta_metric` is also
always reported for comparability.

The sign-flip test statistic uses the arithmetic mean (not Huber) because the
mean has the correct null distribution under sign flipping, whereas the Huber
estimator can degenerate when all repeat differences are equal.

### BCa bootstrap confidence interval

The 95% BCa (bias-corrected and accelerated) interval uses `M_boot` bootstrap
resamples of the `R_eff` repeat-level deltas. Bias correction (z₀) accounts for
the median deviation of bootstrap estimates from the observed statistic; the
acceleration parameter (a) is estimated via jackknife to correct for
distribution skewness. The statistic for the CI is the Huber M-estimator,
matching the `delta_lsi` point estimate.

BCa intervals are available from tier B (`R_eff ≥ 10`). Below that threshold,
the bootstrap distribution is too sparse to estimate the acceleration reliably.

```{r delta-lsi-components}
# Slot-level access to all DLSI outputs
...
**Paired** (recommended): both fits use identical fold structures. Per-repeat
differences δᵣ = m_naive,r − m_guarded,r are computed directly, eliminating
fold-to-fold variance from the test statistic. `R_eff` equals the number of
repeats. Pairing is validated by comparing sorted test-set indices for each
fold element — equal repeat counts with differing index patterns produce a
warning and fall back to unpaired.

**Unpaired**: the two fits have different fold structures or different numbers
of repeats (e.g., naive uses random splits, guarded uses subject-grouped
splits). Only the overall performance summaries are compared; `R_eff = 0` and
all inference is disabled. This mode is appropriate when the splitting scheme
itself is part of what the comparison is testing.

```{r delta-lsi-unpaired, warning=TRUE}
# Unpaired: naive uses random row-wise splits, guarded uses subject-grouped splits
...
|------|------|-------------|
| `@metric` | character | Metric name used |
| `@exchangeability` | character | Always `"iid"` (future: `"block"`) |
| `@tier` | character | Inference tier label |
| `@R_eff` | integer | Effective paired repeats (0 if unpaired) |
| `@delta_metric` | numeric | Arithmetic mean performance gap |
| `@delta_lsi` | numeric | Huber-robust performance gap |
| `@delta_metric_ci` | numeric[2] | 95% BCa CI for `delta_metric` |
| `@delta_lsi_ci` | numeric[2] | 95% BCa CI for `delta_lsi` |
| `@p_value` | numeric | Sign-flip p-value (NA if tier D or unpaired) |
| `@inference_ok` | logical | TRUE only at tier A (`R_eff ≥ 20`) |
| `@folds_naive` | data.frame | Per-fold: fold, metric, delta_lsi |
| `@folds_guarded` | data.frame | Per-fold: fold, metric, delta_lsi |
| `@repeats_naive` | data.frame | Per-repeat: repeat_id, metric |
| `@repeats_guarded` | data.frame | Per-repeat: repeat_id, metric |
| `@info` | list | Metadata: paired, R_naive, R_guarded, higher_is_better, … |
...
substrate for diagnostic visualisation — plot `metric` by `repeat_id` for naive
and guarded to see whether the gap is consistent across repeats or driven by
a few outlier repeats.
...
1. Run `audit_leakage(fit_naive)` to detect leakage and diagnose which
   mechanism is responsible.
2. Build a guarded pipeline that addresses those mechanisms, then run
   `delta_lsi(fit_naive, fit_guarded)` to quantify how much inflation the
