test_that("make_splits validates required columns and time types", {
  df <- make_class_df(6)
  expect_message(make_splits_quiet(df, outcome = "outcome",
                                   mode = "subject_grouped", group = NULL,
                                   v = 2, seed = 1),
                 "No 'group' column provided")
  expect_error(make_splits_quiet(df, outcome = "outcome",
...
test_that("make_splits supports compact assignments and nested splits", {
  df <- make_class_df(12)
  ...
  nested <- make_splits_quiet(df, outcome = "outcome",
                              mode = "subject_grouped", group = "subject",
...
test_that("make_splits errors when no valid folds are produced", {
  df <- make_class_df(4)
  expect_error(make_splits_quiet(df, outcome = "outcome",
                                 mode = "time_series", time = "time",
...
test_that("make_splits supports matrix and SummarizedExperiment inputs", {
  ...
  expect_message({
    splits <- make_splits(mat, outcome = NULL,
                          mode = "subject_grouped", group = NULL,
                          v = 2, seed = 1)
  }, "No 'group' column provided")
  ...
  splits_se <- make_splits(se, outcome = "outcome",
                           mode = "subject_grouped", group = "subject",
