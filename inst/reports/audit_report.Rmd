...
Note: By default, this test keeps predictions fixed and permutes labels, so the p-value
quantifies prediction-label association rather than a full refit null. It does NOT diagnose
...
if (!is.null(pg) && nrow(pg) > 0) {
  ...
  cat(sprintf("Permutation method: %s\n\n",
              if (identical(perm_method, "refit")) "refit per permutation" else "fixed predictions"))
  knitr::kable(pg)
} else {
