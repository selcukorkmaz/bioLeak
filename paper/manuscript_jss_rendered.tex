...
splits
\end{CodeInput}
\input{generated/workflow-splits.tex}
...
summary(fit_leaky)
\end{CodeInput}
\input{generated/workflow-fit-leaky.tex}
...
\end{CodeInput}
\input{generated/workflow-audit-guarded.tex}

\noindent
In this example, the audit detects only modest predictive signal and does not flag subject overlap, batch-related structure, or strong target leakage. The duplicate-detection component does identify several highly similar train--test pairs in the small synthetic reference matrix, illustrating that this audit module can be sensitive even in toy examples and should be interpreted in the context of the chosen similarity threshold and feature representation.
...
plan, fits guarded and leaky comparators, audits the guarded fit, quantifies
...
A typical \texttt{delta\_lsi()} analysis compares a leaky and a guarded fit
built from the same \texttt{LeakSplits} object. The stored case-study result
distributed with the paper can be reloaded and summarized as follows:

\begin{CodeInput}
inflation <- readRDS("paper/casestudy_results.rds")$delta_lsi$delta_lsi_obj

summary(inflation)
\end{CodeInput}
\input{generated/delta-lsi-summary.tex}
...
model is fitted once on the original labels and the permutation test shuffles
labels against the fixed predictions. This design tests whether the
prediction-label association exceeds chance levels, but it does not
distinguish between genuine predictive signal and signal attributable to
leakage. As a consequence, the clean baseline (\texttt{none}) also rejects at
high rates when real signal is present ($s > 0$). The leakage-specific
detection power is therefore best assessed at $s = 0$, where no real signal
exists and the only source of above-chance performance is the injected
leakage mechanism.
...
defaults. The main differences are: (i)~the paper generates
\texttt{peek\_norm} leakage as $y + \varepsilon$ with $\varepsilon \sim
\mathcal{N}(0, 0.09)$, whereas the package function uses a z-scored binary
outcome; (ii)~the paper generates \texttt{lookahead} leakage by shifting a
continuous biomarker rather than the binary outcome; and (iii)~the paper adds
AR(1) noise ($\rho = 0.9$) to the latent linear predictor when $s > 0$. These
choices were made to produce leakage signals of graded strength; readers who
wish to reproduce the manuscript results exactly should use the paper script,
not the package helper. The main result object, \texttt{paper/sim\_results\_all.rds},
...
names(cs)
\end{CodeInput}
\input{generated/casestudy-names.tex}
...
the leaky and guarded case-study pipelines described in Section~7. Both fits
share the same batch-blocked fold structure with 20 repeats, enabling paired
repeat-level inference.

\input{generated/appendix-delta-lsi-summary.tex}
